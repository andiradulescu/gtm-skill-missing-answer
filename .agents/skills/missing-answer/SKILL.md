...
5. `Preview instructions`: give review-only instructions for loading the real target page, applying the smallest local rendered-page edit, preserving its design language, and showing before, changed element, and resulting page. Create the sibling local HTML preview artifact requested by the prompt and label it clearly as a proposed draft. Do not alter the live site.
...
Finish only when exactly one finding contains two or more independent public question excerpts from distinct source URLs, source retrieval dates, checked website evidence, an officially verified answer, confidence, one specific page change, review-only preview instructions, the requested sibling local HTML draft preview, and explicit limitations, with no personal data or external mutation.
